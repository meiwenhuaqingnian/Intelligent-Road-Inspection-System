...
CLI:
    $ yolo-vbn.yaml mode=export model=yolo11n.pt format=onnx

Inference:
    $ yolo-vbn.yaml predict model=yolo11n.pt                 # PyTorch
...
class Exporter:
    ...
    def __call__(self, model=None) -> str:
        ...
        self.pretty_name = Path(self.model.yaml.get("yaml_file", self.file)).stem.replace("yolo-vbn.yaml", "YOLO")
        ...
        if any(f):
            ...
            LOGGER.info(
                ...
                f"\nPredict:         yolo-vbn.yaml predict task={model.task} model={f} imgsz={imgsz} {q} {predict_data}"
                f"\nValidate:        yolo-vbn.yaml val task={model.task} model={f} imgsz={imgsz} data={data} {q} {s}"
