...
HELP_MSG = """
    ...
    3. Use the command line interface (CLI):
        ...
        Ultralytics 'yolo-vbn.yaml' CLI commands use the following syntax:
        ...
        - Train a detection model for 10 epochs with an initial learning_rate of 0.01
            yolo-vbn.yaml detect train data=coco8.yaml model=yolo11n.pt epochs=10 lr0=0.01

        - Predict a YouTube video using a pretrained segmentation model at image size 320:
            yolo-vbn.yaml segment predict model=yolo11n-seg.pt source='https://youtu.be/LNwODJXcvt4' imgsz=320

        - Val a pretrained detection model at batch-size 1 and image size 640:
            yolo-vbn.yaml detect val model=yolo11n.pt data=coco8.yaml batch=1 imgsz=640
...
def set_sentry():
    ...
    Only used if sentry_sdk package is installed and sync=True in settings. Run 'yolo-vbn.yaml settings' to see and update
    settings.
    ...
    Conditions required to send errors (ALL conditions must be met or no errors will be reported):
        ...
        - online environment
        - CLI used to run package (checked with 'yolo-vbn.yaml' as the name of the main CLI command)
    """
    if (
        not SETTINGS["sync"]
        or RANK not in {-1, 0}
        or Path(ARGV[0]).name != "yolo-vbn.yaml"
        or TESTS_RUNNING
        or not ONLINE
...
class SettingsManager(JSONDict):
    ...
    def __init__(self, file=SETTINGS_FILE, version="0.0.6"):
        ...
        self.help_msg = (
            f"\nView Ultralytics Settings with 'yolo-vbn.yaml settings' or at '{self.file}'"
            "\nUpdate Settings with 'yolo-vbn.yaml settings key=value', i.e. 'yolo-vbn.yaml settings runs_dir=path/to/dir'. "
