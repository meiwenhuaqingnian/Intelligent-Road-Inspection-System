...
def check_imgsz(imgsz, stride=32, min_dim=1, max_dim=2, floor=0):
    ...
    if len(imgsz) > max_dim:
        msg = (
            ...
            "or an integer, i.e. 'yolo-vbn.yaml export imgsz=640,480' or 'yolo-vbn.yaml export imgsz=640'"
        )
        if max_dim != 1:
...
def check_imshow(warn=False):
    ...
    try:
        ...
        cv2.imshow("test_images", np.zeros((8, 8, 3), dtype=np.uint8))  # show a small 8-pixel image
        cv2.waitKey(1)
