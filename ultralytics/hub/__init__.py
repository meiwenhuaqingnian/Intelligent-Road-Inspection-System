...
def login(api_key: str = None, save: bool = True) -> bool:
    ...
    if client.authenticated:
        ...
        LOGGER.info(f"{PREFIX}Get API key from {api_key_url} and then run 'yolo-vbn.yaml login API_KEY'")
        return False


def logout():
    """
    Log out of Ultralytics HUB by removing the API key from the settings file. To log in again, use 'yolo-vbn.yaml login'.
    ...
    """
    SETTINGS["api_key"] = ""
    LOGGER.info(f"{PREFIX}logged out ✅. To log in again, use 'yolo-vbn.yaml login'.")
