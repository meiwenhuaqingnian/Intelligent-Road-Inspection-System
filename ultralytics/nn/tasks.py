...
def torch_safe_load(weight, safe_only=False):
    ...
    try:
        with temporary_modules(
            modules={
                "ultralytics.yolo-vbn.yaml.utils": "ultralytics.utils",
                "ultralytics.yolo-vbn.yaml.v8": "ultralytics.models.yolo-vbn.yaml",
                "ultralytics.yolo-vbn.yaml.data": "ultralytics.data",
            },
            attributes={
        ...
        if e.name == "models":
            raise TypeError(
                emojis(
                    ...
                    f"run a command with an official Ultralytics model, i.e. 'yolo-vbn.yaml predict model=yolo11n.pt'"
                )
            ) from e
        LOGGER.warning(
...
def guess_model_scale(model_path):
    ...
    """
    try:
        return re.search(r"yolo-vbn.yaml[v]?\d+([nslmx])", Path(model_path).stem).group(1)  # returns n, s, m, l, or x
