...
def check_det_dataset(dataset, autodownload=True):
    ...
    for k in "train", "val", "test_images", "minival":
...
def check_cls_dataset(dataset, split=""):
    ...
    Args:
        ...
        split (str, optional): The split of the dataset. Either 'val', 'test_images', or ''.

    Returns:
        (dict): A dictionary containing the following keys:
            - 'train' (Path): The directory path containing the training set of the dataset.
            - 'val' (Path): The directory path containing the validation set of the dataset.
            - 'test_images' (Path): The directory path containing the test_images set of the dataset.
    ...
    )  # data/test_images or data/val
    test_set = data_dir / "test_images" if (data_dir / "test_images").exists() else None  # data/val or data/test_images
    if split == "val" and not val_set:
        LOGGER.warning("WARNING ⚠️ Dataset 'split=val' not found, using 'split=test_images' instead.")
        val_set = test_set
    elif split == "test_images" and not test_set:
        LOGGER.warning("WARNING ⚠️ Dataset 'split=test_images' not found, using 'split=val' instead.")
        test_set = val_set
    ...
    # Print to console
    for k, v in {"train": train_set, "val": val_set, "test_images": test_set}.items():
    ...
    return {"train": train_set, "val": val_set, "test_images": test_set, "nc": nc, "names": names}
...
class HUBDatasetStats:
    ...
    def get_json(self, save=False, verbose=False):
        ...
        for split in "train", "val", "test_images":
...
def autosplit(path=DATASETS_DIR / "coco8/images", weights=(0.9, 0.1, 0.0), annotated_only=False):
    """
    Automatically split a dataset into train/val/test_images splits and save the resulting splits into autosplit_*.txt files.

    Args:
        path (Path, optional): Path to images directory.
        weights (list | tuple, optional): Train, validation, and test_images split fractions.
