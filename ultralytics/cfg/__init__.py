...
SOLUTIONS_HELP_MSG = f"""
    Arguments received: {str(["yolo-vbn.yaml"] + ARGV[1:])}. Ultralytics 'yolo-vbn.yaml solutions' usage overview:

        yolo-vbn.yaml solutions SOLUTION ARGS
    ...
    1. Call object counting solution
        yolo-vbn.yaml solutions count source="path/to/video.mp4" region="[(20, 400), (1080, 400), (1080, 360), (20, 360)]"

    2. Call heatmaps solution
        yolo-vbn.yaml solutions heatmap colormap=cv2.COLORMAP_PARULA model=yolo11n.pt

    3. Call queue management solution
        yolo-vbn.yaml solutions queue region="[(20, 400), (1080, 400), (1080, 360), (20, 360)]" model=yolo11n.pt

    4. Call workouts monitoring solution for push-ups
        yolo-vbn.yaml solutions workout model=yolo11n-pose.pt kpts=[6, 8, 10]

    5. Generate analytical graphs
        yolo-vbn.yaml solutions analytics analytics_type="pie"
    
    6. Track objects within specific zones
        yolo-vbn.yaml solutions trackzone source="path/to/video.mp4" region="[(150, 150), (1130, 150), (1130, 570), (150, 570)]"
        
    7. Streamlit real-time webcam inference GUI
        yolo-vbn.yaml streamlit-predict
    """
CLI_HELP_MSG = f"""
    Arguments received: {str(["yolo-vbn.yaml"] + ARGV[1:])}. Ultralytics 'yolo-vbn.yaml' commands use the following syntax:

        yolo-vbn.yaml TASK MODE ARGS

        Where   TASK (optional) is one of {TASKS}
                MODE (required) is one of {MODES}
                ARGS (optional) are any number of custom 'arg=value' pairs like 'imgsz=320' that override defaults.
                    See all ARGS at https://docs.ultralytics.com/usage/cfg or with 'yolo-vbn.yaml cfg'

    1. Train a detection model for 10 epochs with an initial learning_rate of 0.01
        yolo-vbn.yaml train data=coco8.yaml model=yolo11n.pt epochs=10 lr0=0.01

    2. Predict a YouTube video using a pretrained segmentation model at image size 320:
        yolo-vbn.yaml predict model=yolo11n-seg.pt source='https://youtu.be/LNwODJXcvt4' imgsz=320

    3. Val a pretrained detection model at batch-size 1 and image size 640:
        yolo-vbn.yaml val model=yolo11n.pt data=coco8.yaml batch=1 imgsz=640

    4. Export a YOLO11n classification model to ONNX format at image size 224 by 128 (no TASK required)
        yolo-vbn.yaml export model=yolo11n-cls.pt format=onnx imgsz=224,128

    5. Ultralytics solutions usage
        yolo-vbn.yaml solutions count or in {list(SOLUTION_MAP.keys())[1:-1]} source="path/to/video.mp4"

    6. Run special commands:
        yolo-vbn.yaml help
        yolo-vbn.yaml checks
        yolo-vbn.yaml version
        yolo-vbn.yaml settings
        yolo-vbn.yaml copy-cfg
        yolo-vbn.yaml cfg
        yolo-vbn.yaml solutions help
...
def handle_yolo_hub(args: List[str]) -> None:
    ...
    Examples:
        ```bash
        yolo-vbn.yaml login YOUR_API_KEY
        ```
...
def handle_yolo_solutions(args: List[str]) -> None:
    ...
    # Get solution name
    if not args:
        LOGGER.warning("⚠️ No solution name provided. i.e `yolo-vbn.yaml solutions count`. Defaulting to 'count'.")
        args = ["count"]
    if args[0] == "help":
...
def entrypoint(debug: str = "") -> None:
    ...
    for a in merge_equals_args(args):  # merge spaces around '=' sign
        ...
        if "=" in a:
            ...
            overrides[a] = True  # auto-True for default bool args, i.e. 'yolo-vbn.yaml show' sets show=True
...
def copy_default_cfg() -> None:
    ...
    Examples:
        ...
        #   yolo-vbn.yaml cfg='/path/to/current/directory/default_copy.yaml' imgsz=320 batch=8
    ...
    LOGGER.info(
        ...
        f"Example YOLO command with this new custom cfg:\n    yolo-vbn.yaml cfg='{new_file}' imgsz=320 batch=8"
    )


if __name__ == "__main__":
    # Example: entrypoint(debug='yolo-vbn.yaml predict model=yolo11n.pt')
    entrypoint(debug="")
