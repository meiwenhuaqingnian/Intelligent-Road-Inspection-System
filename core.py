"""
道路损伤识别与定位核心模块 (边缘端智能控制版 - 面积占比评估升级版)
特性: 状态机按需拉起推理 / 模型懒加载 / 异步心跳上报 / 基于面积占比的严重度判断
"""
...
REPORT_URL = "http://39.102.84.218:5000/api/report_pi_url"
...
def upload_to_cloud_async(frame: np.ndarray, lon: float, lat: float, severity: int) -> None:
    def _upload():
        try:
            data = {'longitude': lon, 'latitude': lat, 'severity': severity, 'timestamp': time.time(),
...
class YOLO_ONNX_Engine:
    def __init__(self, onnx_model: str, confidence_thres: float = 0.3, iou_thres: float = 0.5):
        ...
        self.session = ort.InferenceSession(self.onnx_model, sess_options=sess_options,
    ...
    def predict(self, frame: np.ndarray):
        ...
        for i in range(output_array.shape[0]):
            classes_scores = output_array[i][4:-1]
        ...
        total_img_area = self.img_height * self.img_width
        ...
        damage_ratio = total_damage_area / total_img_area if total_img_area > 0 else 0

        if damage_ratio == 0:
            severity_level = 0
            severity_text = "None"
            color = (0, 255, 0)
        elif damage_ratio <= THRES_MILD:
            severity_level = 1
            severity_text = "Mild"
            color = (0, 255, 255)
        elif damage_ratio <= THRES_MODERATE:
            severity_level = 2
        ...
        text = f"Severity: {severity_text} ({damage_ratio:.2%})"
        cv2.putText(annotated_frame, text, (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2, cv2.LINE_AA)

        return annotated_frame, severity_level
...
def run_command_polling() -> None:
    ...
    while True:
        try:
            resp = requests.get(CLOUD_COMMAND_URL, timeout=5)
...
def run_heartbeat_loop() -> None:
    ...
    while True:
        ...
        # 无论是否有坐标，都要填一个兜底的值发给云端
        data = {
            'longitude': current_lon if current_lon is not None else '',
            'latitude': current_lat if current_lat is not None else ''
        }
        ...
        try:
            ...
            if frame_to_send is not None:
                ...
                if success:
                    ...
                    requests.post(CLOUD_STREAM_URL, files=files, data=data, timeout=5)
                else:
                    # 画面编码失败，只发数据表活
                    requests.post(CLOUD_STREAM_URL, data=data, timeout=5)
            # 如果没有画面，也强行发空壳请求表活，打破死锁陷阱
            else:
                requests.post(CLOUD_STREAM_URL, data=data, timeout=5)
        except Exception as e:
            pass  # 网络暂时不通没关系，下个循环继续尝试
...
def run_map_loop() -> None:
    ...
    map_response = requests.get("https://restapi.amap.com/v3/staticmap",
                                params={"key": WEB_SERVICE_KEY, "location": f"{center_lon},{center_lat}",
                                        "zoom": ZOOM_LEVEL, "size": f"{MAP_WIDTH}*{MAP_HEIGHT}"}, timeout=10)
    ...
    while True:
        current_lon, current_lat = get_location()
        with lock:
            current_severity = latest_detection_count
            frame_to_save = latest_frame.copy() if latest_frame is not None else None

        if current_lon is not None and current_lat is not None and current_severity > 0:
            ...
            if 0 <= px < MAP_WIDTH and 0 <= py < MAP_HEIGHT:
                ...
                if my_zone is None:
                    my_zone = (px, py)
                    zones[my_zone] = current_severity
                else:
                    zones[my_zone] = max(zones[my_zone], current_severity)
                    ...
                    if my_zone not in saved_severe_locations and frame_to_save is not None:
                        ...
                        if severity >= 2:
                            save_path = os.path.join(SAVE_DIR,
                                                     f"severe_lon{current_lon:.6f}_lat{current_lat:.6f}_{time.strftime('%Y%m%d_%H%M%S')}.jpg")
                            cv2.imwrite(save_path, frame_to_save)
                            cleanup_old_records(SAVE_DIR)
                        saved_severe_locations.add(my_zone)
                    if severity >= 3:
                        cv2.circle(base_map_img, my_zone, radius=12, color=(0, 0, 255), thickness=-1)
                    elif severity == 2:
                        cv2.circle(base_map_img, my_zone, radius=9, color=(0, 165, 255), thickness=-1)
                    else:
                        cv2.circle(base_map_img, my_zone, radius=6, color=(0, 255, 255), thickness=-1)

        with lock:
            latest_map = base_map_img.copy() if base_map_img is not None else None
        time.sleep(0.05)
...
def run_detection_loop() -> None:
    ...
    while True:
        ...
        if global_command == "RUNNING":
            ...
            annotated_frame, severity_level = model.predict(frame)
            ...
            cv2.putText(annotated_frame, status_text, (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 255), 2,
                        cv2.LINE_AA)
            time.sleep(0.05)

        with lock:
            latest_frame = annotated_frame
            latest_detection_count = severity_level
...
if __name__ == "__main__":
    ...
    pi_url = f"https://{local_ip}:8080"
    print(f"[网络] 本机副驾驶定位网址为: {pi_url}")


    def report_url_to_cloud():
        while True:
            try:
                requests.post(REPORT_URL, json={"url": pi_url}, timeout=5, verify=False)
                print("[网络] 🌐 成功将网址汇报给阿里云服务器！")
                break
            except Exception as e:
                print(f"[网络] ⚠️ 汇报失败，原因: {e} \n(1秒后重试...)")
                time.sleep(1)


    threading.Thread(target=report_url_to_cloud, daemon=True).start()
    ...
    threading.Thread(
        target=lambda: flask_app.run(host='0.0.0.0', port=8080, debug=False, use_reloader=False, ssl_context='adhoc'),
        daemon=True).start()
